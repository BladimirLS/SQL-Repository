...
EXECUTE Sales.InsertUpdateSalesPerson @FirstName = 'Bruce', 
								      @LastName = 'Wayne', 
								      @SalaryHr = 545, 
								      @ManagerId = 1, 
								      @StartDate = NULL;
...
IF EXISTS (SELECT 1 FROM Sales.SalesPerson WHERE Email = 'BruceWayne@ABCCompany.com')
	BEGIN
		PRINT 'Bruce is already here!';
	END
...
IF EXISTS (SELECT * FROM Sales.SalesPerson WHERE Email = 'BruceWayne@ABCCompany.com')
	BEGIN
		PRINT 'Bruce is already here!';
	END
